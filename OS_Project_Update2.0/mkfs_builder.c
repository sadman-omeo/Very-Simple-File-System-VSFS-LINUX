// mkfs_builder_refactored.c
// Refactor of your mkfs_builder to match the MiniVSFS spec exactly.
// - 116-byte superblock (packed), magic 0x4D565346, checksum at end
// - 128-byte inode, 12 direct pointers, CRC in last 8 bytes
// - 64-byte dirent (inode_no,u8 type,name[58],u8 checksum)
// - CLI: --image <out.img> --size-kib 180..4096 (multiple of 4) --inodes 128..512
// - Layout: [0]=SB,[1]=inode_bmap,[2]=data_bmap,[3..]=inode_table,[..]=data_region
// - Root inode #1 with '.' and '..', 1 data block allocated, link=2, size=2*64
...
typedef struct {
    ...
    uint32_t flags;               // 0
    uint32_t checksum;            // crc32(superblock[0..4091]) (field must be last)
} superblock_t;
#pragma pack(pop)
...
typedef struct {
    ...
    uint32_t proj_id;      // group ID/project id (set 0)
    uint32_t uid16_gid16;  // 0
    uint64_t xattr_ptr;    // 0
    uint64_t inode_crc;    // CRC (low 4 bytes), last field
} inode_t;
#pragma pack(pop)
...
typedef struct {
    ...
    char     name[58];     // may not be NUL-terminated
...
int main(int argc, char** argv){
    ...
    // Allocate a full zeroed image in memory
    ...
    // inode #1 used
    set_bit(blk1, 0);
    // data region block #0 used (root dir)
    set_bit(blk2, 0);
